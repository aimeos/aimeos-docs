You can also create new items and update or delete existing one them via the Admin JSON API for administration.
...
Additionally, the generated ID by the server is included and all attributes of the items the haven't been sent with the request. The same is returned as list if several items have been sent in the request.
